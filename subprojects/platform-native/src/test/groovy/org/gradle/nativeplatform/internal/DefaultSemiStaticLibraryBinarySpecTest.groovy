...
class DefaultSemiStaticLibraryBinarySpecTest extends Specification {
    @Rule
    ...
    def namingScheme = DefaultBinaryNamingScheme.component("main").withBinaryType("semiStaticLibrary")
    ...
    def tasks = new DefaultSemiStaticLibraryBinarySpec.DefaultTasksCollection(new DefaultBinaryTasksCollection(null, Mock(ITaskFactory)))

    def "has useful string representation"() {
        expect:
        semiStaticLibrary.toString() == "semistatic library 'main:semiStaticLibrary'"
    }

    def getSemiStaticLibrary() {
        TestNativeBinariesFactory.create(SemiStaticLibraryBinarySpec, DefaultSemiStaticLibraryBinarySpec, "test", Mock(ITaskFactory), library, namingScheme, resolver, platform,
            buildType, new DefaultFlavor("flavorOne"))
    }

    def "can set output file"() {
        given:
        final binary = semiStaticLibrary
        def outputFile = Mock(File)

        when:
        binary.semiStaticLibraryFile = outputFile
    ...
    def "can convert binary to a native dependency"() {
        ...
        binary.headerDirs.toString() == "Headers for semistatic library 'main:semiStaticLibrary'"
        binary.semiStaticLibraryFile == outputFile

        and:
        binary.linkFiles.files == [binary.semiStaticLibraryFile] as Set
        binary.linkFiles.buildDependencies.getDependencies(Stub(Task)) == [lifecycleTask] as Set
        binary.linkFiles.toString() == "Link files for semistatic library 'main:semiStaticLibrary'"

        and:
        binary.runtimeFiles.files.isEmpty()
        binary.runtimeFiles.buildDependencies.getDependencies(Stub(Task)) == [] as Set
        binary.runtimeFiles.toString() == "Runtime files for semistatic library 'main:semiStaticLibrary'"
    }
    ...
    def "includes additional link files in native dependency"() {
        ...
        expect:
        binary.semiStaticLibraryFile == outputFile
        binary.linkFiles.files == [binary.semiStaticLibraryFile, linkFile1, linkFile2] as Set
    }

    def "returns null for createSemiStaticLib and builder when none defined"() {
        expect:
        tasks.createSemiStaticLib == null
    }

    def "returns create task when defined"() {
        when:
        final createTask = TestUtil.create(tmpDir).task(CreateSemiStaticLibrary)
        tasks.add(createTask)

        then:
        tasks.createSemiStaticLib == createTask
    }

    private TestFile addSources(SemiStaticLibraryBinarySpec binary, def headerDir) {
